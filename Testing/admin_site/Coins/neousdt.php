...
<body>
        ...
        <aside class="market-list">
            <ul id="market-list">
                <li data-symbol="btcusdt">
                    <strong>NEOUSDT</strong> <span class="price">$0.00</span> <span class="change">(0.00%)</span>
                </li>
        ...
        <main class="chart-area">
            ...
            <div class="trade-columns">
                <div class="buy-column">
                    ...
                    <div id="maxBuyNEO" style="color:grey;">Max Buy NEO: 0.000000</div>
                    <form id="buy-form">                        
                        <label for="buy-amount">Amount (NEO):</label>
                        ...
                        <button type="submit" id="buy-button" >Buy NEO</button>
                    </form>
                </div>
                ...
                <div id="maxSellNEO" style="color:grey;">Max Sell NEO: 0.000000</div>
                    <form id="sell-form">
                        ...
                        <button type="submit" id="sell-button" >Sell NEO</button>
                    </form>
                </div>
            </div>
        </main>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function connectWebSocket() {
                const ws = new WebSocket('wss://stream.binance.com:9443/ws/neousdt@trade'); // Binance NEO/USDT trade stream
            ...
            function calculateMaxAmounts(currentPrice) {
                ...
                const maxBuyNeo = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxBuyNEO').innerText = `Max Buy NEO: ${maxBuyNeo}`;

                // Calculate Max Sell BTC (also based on available USDT)
                const maxSellNeo = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxSellNEO').innerText = `Max Sell NEO: ${maxSellNeo}`;
            }
            function validateAmount(type) {
                const amountInput = document.getElementById(type + '-amount');
                const maxAmount = parseFloat(document.getElementById(type === 'buy' ? 'maxBuyNEO' : 'maxSellNEO').innerText.replace('Max ' + (type === 'buy' ? 'Buy' : 'Sell') + ' NEO: ', ''));

                if (parseFloat(amountInput.value) > maxAmount) {
                    amountInput.value = maxAmount.toFixed(6); // Set value to max allowed if exceeded
                    alert(`The amount cannot exceed the maximum ${type === 'buy' ? 'buy' : 'sell'} limit of ${maxAmount} NEO.`);
                }
            }
    ...
    function validateForm(event, formId) {
        ...
        const maxAmount = parseFloat(document.getElementById(formId === 'buy-form' ? 'maxBuyNEO' : 'maxSellNEO').innerText.replace('Max ' + (formId === 'buy-form' ? 'Buy' : 'Sell') + ' NEO: ', ''));
        ...
        if (amount < 1 || amount > maxAmount) {
            event.preventDefault();
            alert(`Please enter an amount between 1 NEO and the maximum allowed: ${maxAmount} NEO.`);
            return;
        }
    ...
    function startCountdown(overlay, timeRemaining, startPrice, amount, orderDirection) {
        ...
        const ws = new WebSocket('wss://stream.binance.com:9443/ws/neousdt@trade');
        ws.onmessage = event => {
            const data = JSON.parse(event.data);
            const currentPrice = parseFloat(data.p).toFixed(4);
...
<script>
        ...
        let symbol = 'NEOUSDT'; // Initially set to BTCUSDT
        let interval = '1m';
    ...
    // Connecting to Binance WebSocket for NEOUSDT order book
    const socket = new WebSocket('wss://stream.binance.com:9443/ws/neousdt@depth');
