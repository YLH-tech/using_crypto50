...
<body>
        ...
        <main class="chart-area">
            ...
            <div class="trade-columns">
                <div class="buy-column">
                    ...
                    <div id="maxBuyLINK" style="color:grey;">Max Buy LINK: 0.000000</div>
                    <form id="buy-form">                        
                        <label for="buy-amount">Amount (LINK):</label>
                        ...
                        <button type="submit" id="buy-button" >Buy LINK</button>
                    </form>
                </div>
                ...
                <div id="maxSellLINK" style="color:grey;">Max Sell LINK: 0.000000</div>
                    <form id="sell-form">
                        
                        <label for="sell-amount">Amount (LINK):</label>
                        ...
                        <button type="submit" id="sell-button" >Sell LINK</button>
                    </form>
                </div>
            </div>
        </main>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function connectWebSocket() {
                const ws = new WebSocket('wss://stream.binance.com:9443/ws/linkusdt@trade'); // Binance LINK/USDT trade stream
            ...
            function calculateMaxAmounts(currentPrice) {
                ...
                const maxBuyLink = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxBuyLINK').innerText = `Max Buy LINK: ${maxBuyLink}`;

                // Calculate Max Sell BTC (also based on available USDT)
                const maxSellLink = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxSellLINK').innerText = `Max Sell LINK: ${maxSellLink}`;
            }
            function validateAmount(type) {
                const amountInput = document.getElementById(type + '-amount');
                const maxAmount = parseFloat(document.getElementById(type === 'buy' ? 'maxBuyLINK' : 'maxSellLINK').innerText.replace('Max ' + (type === 'buy' ? 'Buy' : 'Sell') + ' LINK: ', ''));

                if (parseFloat(amountInput.value) > maxAmount) {
                    amountInput.value = maxAmount.toFixed(6); // Set value to max allowed if exceeded
                    alert(`The amount cannot exceed the maximum ${type === 'buy' ? 'buy' : 'sell'} limit of ${maxAmount} LINK.`);
                }
            }
    ...
    function validateForm(event, formId) {
        ...
        const maxAmount = parseFloat(document.getElementById(formId === 'buy-form' ? 'maxBuyLINK' : 'maxSellLINK').innerText.replace('Max ' + (formId === 'buy-form' ? 'Buy' : 'Sell') + ' LINK: ', ''));
        ...
        if (amount < 0.5 || amount > maxAmount) {
            event.preventDefault();
            alert(`Please enter an amount between 0.5 LINK and the maximum allowed: ${maxAmount} LINK.`);
            return;
        }
...
<script>
        ...
        let symbol = 'LINKUSDT'; // Initially set to BTCUSDT
        let interval = '1m';
    ...
    // Connecting to Binance WebSocket for LINKUSDT order book
    const socket = new WebSocket('wss://stream.binance.com:9443/ws/linkusdt@depth');
