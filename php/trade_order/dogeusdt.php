...
<body>
        ...
        <main class="chart-area">
            ...
            <div class="trade-columns">
                <div class="buy-column">
                    ...
                    <div id="maxBuyDOGE" style="color:grey;">Max Buy DOGE: 0.000000</div>
                    <form id="buy-form">                        
                        <label for="buy-amount">Amount (DOGE):</label>
                        ...
                        <button type="submit" id="buy-button" >Buy DOGE</button>
                    </form>
                </div>
                ...
                <div id="maxSellDOGE" style="color:grey;">Max Sell DOGE: 0.000000</div>
                    <form id="sell-form">
                        
                        <label for="sell-amount">Amount (DOGE):</label>
                        ...
                        <button type="submit" id="sell-button" >Sell DOGE</button>
                    </form>
                </div>
            </div>
        </main>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function connectWebSocket() {
                const ws = new WebSocket('wss://stream.binance.com:9443/ws/dogeusdt@trade'); // Binance DOGE/USDT trade stream

                ws.onmessage = event => {
                    const data = JSON.parse(event.data);
                    const currentPrice = parseFloat(data.p).toFixed(5);
            ...
            function calculateMaxAmounts(currentPrice) {
                ...
                const maxBuyDoge = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxBuyDOGE').innerText = `Max Buy DOGE: ${maxBuyDoge}`;

                // Calculate Max Sell BTC (also based on available USDT)
                const maxSellDoge = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxSellDOGE').innerText = `Max Sell DOGE: ${maxSellDoge}`;
            }
            function validateAmount(type) {
                const amountInput = document.getElementById(type + '-amount');
                const maxAmount = parseFloat(document.getElementById(type === 'buy' ? 'maxBuyDOGE' : 'maxSellDOGE').innerText.replace('Max ' + (type === 'buy' ? 'Buy' : 'Sell') + ' DOGE: ', ''));

                if (parseFloat(amountInput.value) > maxAmount) {
                    amountInput.value = maxAmount.toFixed(6); // Set value to max allowed if exceeded
                    alert(`The amount cannot exceed the maximum ${type === 'buy' ? 'buy' : 'sell'} limit of ${maxAmount} DOGE.`);
                }
            }
    ...
    function validateForm(event, formId) {
        ...
        const maxAmount = parseFloat(document.getElementById(formId === 'buy-form' ? 'maxBuyDOGE' : 'maxSellDOGE').innerText.replace('Max ' + (formId === 'buy-form' ? 'Buy' : 'Sell') + ' DOGE: ', ''));
        ...
        if (amount < 50 || amount > maxAmount) {
            event.preventDefault();
            alert(`Please enter an amount between 50 DOGE and the maximum allowed: ${maxAmount} DOGE.`);
            return;
        }
    ...
    function showCountdownOverlay(formId, amount) {
        ...
        const currentPrice = parseFloat(document.getElementById('buy-price').innerText.replace('Price (USDT): ', '')).toFixed(5);
    ...
    function showOrderConfirmation(symbol, amount, startPrice, orderDirection) {
        ...
        const endPrice = parseFloat(document.getElementById('realTimePrice').innerText.replace('Current Price: $', '')).toFixed(5);
...
<script>
        ...
        let symbol = 'DOGEUSDT'; // Initially set to BTCUSDT
        let interval = '1m';
    ...
    // Connecting to Binance WebSocket for DOGEUSDT order book
    const socket = new WebSocket('wss://stream.binance.com:9443/ws/dogeusdt@depth');
    ...
    function fillOrderTable(sellOrders, buyOrders) {
        ...
        for (let i = 0; i < 10; i++) { // Updated to 10
            ...
            row.innerHTML = `
                <td style="color: red; font-weight: bold; font-size: 12px;">${price.toFixed(5)}</td>
        ...
        for (let i = 0; i < 10; i++) { // Updated to 10
            ...
            const row = document.createElement('tr');
            row.innerHTML = `
                <td style="color: green; font-weight: bold; font-size: 12px;">${price.toFixed(5)}</td>
                <td style="color: grey; font-weight: bold; font-size: 12px;">${amount.toFixed(5)}</td>
                <td style="color: grey; font-weight: bold; font-size: 12px;">${(total / 1000).toFixed(2)}K</td>
            `;
            orderTableBody.appendChild(row);
