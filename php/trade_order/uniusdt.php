...
<body>
        ...
        <main class="chart-area">
            ...
            <div class="trade-columns">
                <div class="buy-column">
                    ...
                    <div id="maxBuyUNI" style="color:grey;">Max Buy UNI: 0.000000</div>
                    <form id="buy-form">                        
                        <label for="buy-amount">Amount (UNI):</label>
                        ...
                        <button type="submit" id="buy-button" >Buy UNI</button>
                    </form>
                </div>
                ...
                <div id="maxSellUNI" style="color:grey;">Max Sell UNI: 0.000000</div>
                    <form id="sell-form">
                        
                        <label for="sell-amount">Amount (UNI):</label>
                        ...
                        <button type="submit" id="sell-button" >Sell UNI</button>
                    </form>
                </div>
            </div>
        </main>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function connectWebSocket() {
                const ws = new WebSocket('wss://stream.binance.com:9443/ws/uniusdt@trade'); // Binance UNI/USDT trade stream
            ...
            function calculateMaxAmounts(currentPrice) {
                ...
                const maxBuyUni = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxBuyUNI').innerText = `Max Buy UNI: ${maxBuyUni}`;

                // Calculate Max Sell BTC (also based on available USDT)
                const maxSellUni = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxSellUNI').innerText = `Max Sell UNI: ${maxSellUni}`;
            }
            function validateAmount(type) {
                const amountInput = document.getElementById(type + '-amount');
                const maxAmount = parseFloat(document.getElementById(type === 'buy' ? 'maxBuyUNI' : 'maxSellUNI').innerText.replace('Max ' + (type === 'buy' ? 'Buy' : 'Sell') + ' UNI: ', ''));

                if (parseFloat(amountInput.value) > maxAmount) {
                    amountInput.value = maxAmount.toFixed(6); // Set value to max allowed if exceeded
                    alert(`The amount cannot exceed the maximum ${type === 'buy' ? 'buy' : 'sell'} limit of ${maxAmount} UNI.`);
                }
            }
    ...
    function validateForm(event, formId) {
        ...
        const maxAmount = parseFloat(document.getElementById(formId === 'buy-form' ? 'maxBuyUNI' : 'maxSellUNI').innerText.replace('Max ' + (formId === 'buy-form' ? 'Buy' : 'Sell') + ' UNI: ', ''));
        ...
        if (amount < 1 || amount > maxAmount) {
            event.preventDefault();
            alert(`Please enter an amount between 1 UNI and the maximum allowed: ${maxAmount} UNI.`);
            return;
        }
    ...
    function startCountdown(overlay, timeRemaining, startPrice, amount, orderDirection) {
        ...
        const ws = new WebSocket('wss://stream.binance.com:9443/ws/uniusdt@trade');
        ws.onmessage = event => {
            const data = JSON.parse(event.data);
            const currentPrice = parseFloat(data.p).toFixed(4);
...
<script>
        ...
        let symbol = 'UNIUSDT'; // Initially set to BTCUSDT
        let interval = '1m';
    ...
    // Connecting to Binance WebSocket for UNIUSDT order book
    const socket = new WebSocket('wss://stream.binance.com:9443/ws/uniusdt@depth');
