...
<body>
        ...
        <main class="chart-area">
            ...
            <div class="trade-columns">
                <div class="buy-column">
                    ...
                    <div id="maxBuyETH" style="color:grey;">Max Buy ETH: 0.000000</div>
                    <form id="buy-form">                        
                        ...
                        <button type="submit" id="buy-button" >Buy ETH</button>
                    </form>
                </div>
                ...
                <div id="maxSellETH" style="color:grey;">Max Sell ETH: 0.000000</div>
                    <form id="sell-form">
                        
                        <label for="sell-amount">Amount (ETH):</label>
                        ...
                        <button type="submit" id="sell-button" >Sell ETH</button>
                    </form>
                </div>
            </div>
        </main>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function connectWebSocket() {
                const ws = new WebSocket('wss://stream.binance.com:9443/ws/ethusdt@trade'); // Binance ETH/USDT trade stream
            ...
            function calculateMaxAmounts(currentPrice) {
                ...
                const maxBuyEth = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxBuyETH').innerText = `Max Buy ETH: ${maxBuyEth}`;

                // Calculate Max Sell BTC (also based on available USDT)
                const maxSellEth = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxSellETH').innerText = `Max Sell ETH: ${maxSellEth}`;
            }
            function validateAmount(type) {
                const amountInput = document.getElementById(type + '-amount');
                const maxAmount = parseFloat(document.getElementById(type === 'buy' ? 'maxBuyETH' : 'maxSellETH').innerText.replace('Max ' + (type === 'buy' ? 'Buy' : 'Sell') + ' ETH: ', ''));

                if (parseFloat(amountInput.value) > maxAmount) {
                    amountInput.value = maxAmount.toFixed(6); // Set value to max allowed if exceeded
                    alert(`The amount cannot exceed the maximum ${type === 'buy' ? 'buy' : 'sell'} limit of ${maxAmount} ETH.`);
                }
            }
    ...
    function validateForm(event, formId) {
        ...
        const maxAmount = parseFloat(document.getElementById(formId === 'buy-form' ? 'maxBuyETH' : 'maxSellETH').innerText.replace('Max ' + (formId === 'buy-form' ? 'Buy' : 'Sell') + ' ETH: ', ''));
        ...
        if (amount < 0.005 || amount > maxAmount) {
            event.preventDefault();
            alert(`Please enter an amount between 0.005 ETH and the maximum allowed: ${maxAmount} ETH.`);
            return;
        }
...
<script>
        ...
        let symbol = 'ETHUSDT'; // Initially set to BTCUSDT
        let interval = '1m';
    ...
    // Connecting to Binance WebSocket for ETHUSDT order book
    const socket = new WebSocket('wss://stream.binance.com:9443/ws/ethusdt@depth');
