...
<body>
        ...
        <main class="chart-area">
            ...
            <div class="trade-columns">
                <div class="buy-column">
                    ...
                    <div id="maxBuyTRX" style="color:grey;">Max Buy TRX: 0.000000</div>
                    <form id="buy-form">                        
                        <label for="buy-amount">Amount (TRX):</label>
                        ...
                        <button type="submit" id="buy-button" >Buy TRX</button>
                    </form>
                </div>
                ...
                <div id="maxSellTRX" style="color:grey;">Max Sell TRX: 0.000000</div>
                    <form id="sell-form">
                        
                        <label for="sell-amount">Amount (TRX):</label>
                        ...
                        <button type="submit" id="sell-button" >Sell TRX</button>
                    </form>
                </div>
            </div>
        </main>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function connectWebSocket() {
                const ws = new WebSocket('wss://stream.binance.com:9443/ws/trxusdt@trade'); // Binance TRX/USDT trade stream

                ws.onmessage = event => {
                    const data = JSON.parse(event.data);
                    const currentPrice = parseFloat(data.p).toFixed(6);
            ...
            function calculateMaxAmounts(currentPrice) {
                ...
                const maxBuyTrx = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxBuyTRX').innerText = `Max Buy TRX: ${maxBuyTrx}`;

                // Calculate Max Sell BTC (also based on available USDT)
                const maxSellTrx = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxSellTRX').innerText = `Max Sell TRX: ${maxSellTrx}`;
            }
            function validateAmount(type) {
                const amountInput = document.getElementById(type + '-amount');
                const maxAmount = parseFloat(document.getElementById(type === 'buy' ? 'maxBuyTRX' : 'maxSellTRX').innerText.replace('Max ' + (type === 'buy' ? 'Buy' : 'Sell') + ' TRX: ', ''));

                if (parseFloat(amountInput.value) > maxAmount) {
                    amountInput.value = maxAmount.toFixed(6); // Set value to max allowed if exceeded
                    alert(`The amount cannot exceed the maximum ${type === 'buy' ? 'buy' : 'sell'} limit of ${maxAmount} TRX.`);
                }
            }
    ...
    function validateForm(event, formId) {
        ...
        const maxAmount = parseFloat(document.getElementById(formId === 'buy-form' ? 'maxBuyTRX' : 'maxSellTRX').innerText.replace('Max ' + (formId === 'buy-form' ? 'Buy' : 'Sell') + ' TRX: ', ''));
        ...
        if (amount < 50 || amount > maxAmount) {
            event.preventDefault();
            alert(`Please enter an amount between 50 TRX and the maximum allowed: ${maxAmount} TRX.`);
            return;
        }
    ...
    function showCountdownOverlay(formId, amount) {
        ...
        const currentPrice = parseFloat(document.getElementById('buy-price').innerText.replace('Price (USDT): ', '')).toFixed(6);
...
<script>
        ...
        let symbol = 'TRXUSDT'; // Initially set to BTCUSDT
        let interval = '1m';
    ...
    // Connecting to Binance WebSocket for TRXUSDT order book
    const socket = new WebSocket('wss://stream.binance.com:9443/ws/trxusdt@depth');
