...
<body>
        ...
        <main class="chart-area">
            ...
            <div class="trade-columns">
                <div class="buy-column">
                    ...
                    <div id="maxBuyEOS" style="color:grey;">Max Buy EOS: 0.000000</div>
                    <form id="buy-form">                        
                        <label for="buy-amount">Amount (EOS):</label>
                        ...
                        <button type="submit" id="buy-button" >Buy EOS</button>
                    </form>
                </div>
                ...
                <div id="maxSellEOS" style="color:grey;">Max Sell EOS: 0.000000</div>
                    <form id="sell-form">
                        
                        <label for="sell-amount">Amount (EOS):</label>
                        ...
                        <button type="submit" id="sell-button" >Sell EOS</button>
                    </form>
                </div>
            </div>
        </main>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function connectWebSocket() {
                const ws = new WebSocket('wss://stream.binance.com:9443/ws/eosusdt@trade'); // Binance EOS/USDT trade stream
            ...
            function calculateMaxAmounts(currentPrice) {
                ...
                const maxBuyEos = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxBuyEOS').innerText = `Max Buy EOS: ${maxBuyEos}`;

                // Calculate Max Sell BTC (also based on available USDT)
                const maxSellEos = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxSellEOS').innerText = `Max Sell EOS: ${maxSellEos}`;
            }
            function validateAmount(type) {
                const amountInput = document.getElementById(type + '-amount');
                const maxAmount = parseFloat(document.getElementById(type === 'buy' ? 'maxBuyEOS' : 'maxSellEOS').innerText.replace('Max ' + (type === 'buy' ? 'Buy' : 'Sell') + ' EOS: ', ''));

                if (parseFloat(amountInput.value) > maxAmount) {
                    amountInput.value = maxAmount.toFixed(6); // Set value to max allowed if exceeded
                    alert(`The amount cannot exceed the maximum ${type === 'buy' ? 'buy' : 'sell'} limit of ${maxAmount} EOS.`);
                }
            }
    ...
    function validateForm(event, formId) {
        ...
        const maxAmount = parseFloat(document.getElementById(formId === 'buy-form' ? 'maxBuyEOS' : 'maxSellEOS').innerText.replace('Max ' + (formId === 'buy-form' ? 'Buy' : 'Sell') + ' EOS: ', ''));
        ...
        if (amount < 50 || amount > maxAmount) {
            event.preventDefault();
            alert(`Please enter an amount between 50 EOS and the maximum allowed: ${maxAmount} EOS.`);
            return;
        }
...
<script>
        ...
        let symbol = 'EOSUSDT'; // Initially set to BTCUSDT
        let interval = '1m';
    ...
    // Connecting to Binance WebSocket for EOSUSDT order book
    const socket = new WebSocket('wss://stream.binance.com:9443/ws/eosusdt@depth');
