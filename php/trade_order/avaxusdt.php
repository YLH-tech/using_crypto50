...
<body>
        ...
        <main class="chart-area">
            ...
            <div class="trade-columns">
                <div class="buy-column">
                    ...
                    <div id="maxBuyAVAX" style="color:grey;">Max Buy AVAX: 0.000000</div>
                    <form id="buy-form">                        
                        <label for="buy-amount">Amount (AVAX):</label>
                        ...
                        <button type="submit" id="buy-button" >Buy AVAX</button>
                    </form>
                </div>
                ...
                <div id="maxSellAVAX" style="color:grey;">Max Sell AVAX: 0.000000</div>
                    <form id="sell-form">
                        
                        <label for="sell-amount">Amount (AVAX):</label>
                        ...
                        <button type="submit" id="sell-button" >Sell AVAX</button>
                    </form>
                </div>
            </div>
        </main>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function connectWebSocket() {
                const ws = new WebSocket('wss://stream.binance.com:9443/ws/avaxusdt@trade'); // Binance AVAX/USDT trade stream
            ...
            function calculateMaxAmounts(currentPrice) {
                ...
                const maxBuyAvax = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxBuyAVAX').innerText = `Max Buy AVAX: ${maxBuyAvax}`;

                // Calculate Max Sell BTC (also based on available USDT)
                const maxSellAvax = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxSellAVAX').innerText = `Max Sell AVAX: ${maxSellAvax}`;
            }
            function validateAmount(type) {
                const amountInput = document.getElementById(type + '-amount');
                const maxAmount = parseFloat(document.getElementById(type === 'buy' ? 'maxBuyAVAX' : 'maxSellAVAX').innerText.replace('Max ' + (type === 'buy' ? 'Buy' : 'Sell') + ' AVAX: ', ''));

                if (parseFloat(amountInput.value) > maxAmount) {
                    amountInput.value = maxAmount.toFixed(6); // Set value to max allowed if exceeded
                    alert(`The amount cannot exceed the maximum ${type === 'buy' ? 'buy' : 'sell'} limit of ${maxAmount} AVAX.`);
                }
            }
    ...
    function validateForm(event, formId) {
        ...
        const maxAmount = parseFloat(document.getElementById(formId === 'buy-form' ? 'maxBuyAVAX' : 'maxSellAVAX').innerText.replace('Max ' + (formId === 'buy-form' ? 'AVAX' : 'Sell') + ' BTC: ', ''));
        ...
        if (amount < 0.5 || amount > maxAmount) {
            event.preventDefault();
            alert(`Please enter an amount between 0.5 AVAX and the maximum allowed: ${maxAmount} AVAX.`);
            return;
        }
    ...
    function startCountdown(overlay, timeRemaining, startPrice, amount, orderDirection) {
        ...
        const ws = new WebSocket('wss://stream.binance.com:9443/ws/avaxusdt@trade');
        ws.onmessage = event => {
            const data = JSON.parse(event.data);
            const currentPrice = parseFloat(data.p).toFixed(4);
...
<script>
        ...
        let symbol = 'AVAXUSDT'; // Initially set to BTCUSDT
        let interval = '1m';
    ...
    // Connecting to Binance WebSocket for AVAXUSDT order book
    const socket = new WebSocket('wss://stream.binance.com:9443/ws/avaxusdt@depth');
