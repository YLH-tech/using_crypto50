...
<body>
        ...
        <main class="chart-area">
            ...
            <div class="trade-columns">
                <div class="buy-column">
                    ...
                    <div id="maxBuyDOT" style="color:grey;">Max Buy DOT: 0.000000</div>
                    <form id="buy-form">                        
                        <label for="buy-amount">Amount (DOT):</label>
                        ...
                        <button type="submit" id="buy-button" >Buy DOT</button>
                    </form>
                </div>
                ...
                <div id="maxSellDOT" style="color:grey;">Max Sell DOT: 0.000000</div>
                    <form id="sell-form">
                        
                        <label for="sell-amount">Amount (DOT):</label>
                        ...
                        <button type="submit" id="sell-button" >Sell DOT</button>
                    </form>
                </div>
            </div>
        </main>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function connectWebSocket() {
                const ws = new WebSocket('wss://stream.binance.com:9443/ws/dotusdt@trade'); // Binance DOT/USDT trade stream
            ...
            function calculateMaxAmounts(currentPrice) {
                ...
                const maxBuyDot = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxBuyDOT').innerText = `Max Buy DOT: ${maxBuyDot}`;

                // Calculate Max Sell BTC (also based on available USDT)
                const maxSellDot = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxSellDOT').innerText = `Max Sell DOT: ${maxSellDot}`;
            }
            function validateAmount(type) {
                const amountInput = document.getElementById(type + '-amount');
                const maxAmount = parseFloat(document.getElementById(type === 'buy' ? 'maxBuyDOT' : 'maxSellDOT').innerText.replace('Max ' + (type === 'buy' ? 'Buy' : 'Sell') + ' DOT: ', ''));

                if (parseFloat(amountInput.value) > maxAmount) {
                    amountInput.value = maxAmount.toFixed(6); // Set value to max allowed if exceeded
                    alert(`The amount cannot exceed the maximum ${type === 'buy' ? 'buy' : 'sell'} limit of ${maxAmount} DOT.`);
                }
            }
    ...
    function validateForm(event, formId) {
        ...
        const maxAmount = parseFloat(document.getElementById(formId === 'buy-form' ? 'maxBuyDOT' : 'maxSellDOT').innerText.replace('Max ' + (formId === 'buy-form' ? 'Buy' : 'Sell') + ' DOT: ', ''));
        ...
        if (amount < 5 || amount > maxAmount) {
            event.preventDefault();
            alert(`Please enter an amount between 5 DOT and the maximum allowed: ${maxAmount} DOT.`);
            return;
        }
    ...
    function startCountdown(overlay, timeRemaining, startPrice, amount, orderDirection) {
        ...
        const ws = new WebSocket('wss://stream.binance.com:9443/ws/dotusdt@trade');
        ws.onmessage = event => {
            const data = JSON.parse(event.data);
            const currentPrice = parseFloat(data.p).toFixed(4);
...
<script>
        ...
        let symbol = 'DOTUSDT'; // Initially set to BTCUSDT
        let interval = '1m';
    ...
    // Connecting to Binance WebSocket for DOTUSDT order book
    const socket = new WebSocket('wss://stream.binance.com:9443/ws/dotusdt@depth');
