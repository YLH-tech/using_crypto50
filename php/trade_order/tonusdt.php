...
<body>
        ...
        <main class="chart-area">
            ...
            <div class="trade-columns">
                <div class="buy-column">
                    ...
                    <div id="maxBuyTON" style="color:grey;">Max Buy TON: 0.000000</div>
                    <form id="buy-form">                        
                        <label for="buy-amount">Amount (TON):</label>
                        ...
                        <button type="submit" id="buy-button" >Buy TON</button>
                    </form>
                </div>
                ...
                <div id="maxSellTON" style="color:grey;">Max Sell TON: 0.000000</div>
                    <form id="sell-form">
                        
                        <label for="sell-amount">Amount (TON):</label>
                        ...
                        <button type="submit" id="sell-button" >Sell TON</button>
                    </form>
                </div>
            </div>
        </main>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function connectWebSocket() {
                const ws = new WebSocket('wss://stream.binance.com:9443/ws/tonusdt@trade'); // Binance TON/USDT trade stream
            ...
            function calculateMaxAmounts(currentPrice) {
                ...
                const maxBuyTon = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxBuyTON').innerText = `Max Buy TON: ${maxBuyTon}`;

                // Calculate Max Sell BTC (also based on available USDT)
                const maxSellTon = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxSellTON').innerText = `Max Sell TON: ${maxSellTon}`;
            }
            function validateAmount(type) {
                const amountInput = document.getElementById(type + '-amount');
                const maxAmount = parseFloat(document.getElementById(type === 'buy' ? 'maxBuyTON' : 'maxSellTON').innerText.replace('Max ' + (type === 'buy' ? 'Buy' : 'Sell') + ' TON: ', ''));

                if (parseFloat(amountInput.value) > maxAmount) {
                    amountInput.value = maxAmount.toFixed(6); // Set value to max allowed if exceeded
                    alert(`The amount cannot exceed the maximum ${type === 'buy' ? 'buy' : 'sell'} limit of ${maxAmount} TON.`);
                }
            }
    ...
    function validateForm(event, formId) {
        ...
        const maxAmount = parseFloat(document.getElementById(formId === 'buy-form' ? 'maxBuyTON' : 'maxSellTON').innerText.replace('Max ' + (formId === 'buy-form' ? 'Buy' : 'Sell') + ' TON: ', ''));
        ...
        if (amount < 5 || amount > maxAmount) {
            event.preventDefault();
            alert(`Please enter an amount between 5 TON and the maximum allowed: ${maxAmount} TON.`);
            return;
        }
    ...
    function startCountdown(overlay, timeRemaining, startPrice, amount, orderDirection) {
        ...
        const ws = new WebSocket('wss://stream.binance.com:9443/ws/tonusdt@trade');
        ws.onmessage = event => {
            const data = JSON.parse(event.data);
            const currentPrice = parseFloat(data.p).toFixed(4);
...
<script>
        ...
        let symbol = 'TONUSDT'; // Initially set to BTCUSDT
        let interval = '1m';
    ...
    // Connecting to Binance WebSocket for TONUSDT order book
    const socket = new WebSocket('wss://stream.binance.com:9443/ws/tonusdt@depth');
