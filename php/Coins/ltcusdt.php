...
<body>
        ...
        <main class="chart-area">
            ...
            <div class="trade-columns">
                <div class="buy-column">
                    ...
                    <div id="maxBuyLTC" style="color:grey;">Max Buy LTC: 0.000000</div>
                    <form id="buy-form">                        
                        <label for="buy-amount">Amount (LTC):</label>
                        ...
                        <button type="submit" id="buy-button" >Buy LTC</button>
                    </form>
                </div>
                ...
                <div id="maxSellLTC" style="color:grey;">Max Sell LTC: 0.000000</div>
                    <form id="sell-form">
                        
                        <label for="sell-amount">Amount (LTC):</label>
                        ...
                        <button type="submit" id="sell-button" >Sell LTC</button>
                    </form>
                </div>
            </div>
        </main>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function connectWebSocket() {
                const ws = new WebSocket('wss://stream.binance.com:9443/ws/ltcusdt@trade'); // Binance LTC/USDT trade stream
            ...
            function calculateMaxAmounts(currentPrice) {
                ...
                const maxBuyLtc = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxBuyLTC').innerText = `Max Buy LTC: ${maxBuyLtc}`;

                // Calculate Max Sell BTC (also based on available USDT)
                const maxSellLtc = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxSellLTC').innerText = `Max Sell LTC: ${maxSellLtc}`;
            }
            function validateAmount(type) {
                const amountInput = document.getElementById(type + '-amount');
                const maxAmount = parseFloat(document.getElementById(type === 'buy' ? 'maxBuyLTC' : 'maxSellLTC').innerText.replace('Max ' + (type === 'buy' ? 'Buy' : 'Sell') + ' LTC: ', ''));

                if (parseFloat(amountInput.value) > maxAmount) {
                    amountInput.value = maxAmount.toFixed(6); // Set value to max allowed if exceeded
                    alert(`The amount cannot exceed the maximum ${type === 'buy' ? 'buy' : 'sell'} limit of ${maxAmount} LTC.`);
                }
            }
    ...
    function validateForm(event, formId) {
        ...
        const maxAmount = parseFloat(document.getElementById(formId === 'buy-form' ? 'maxBuyLTC' : 'maxSellLTC').innerText.replace('Max ' + (formId === 'buy-form' ? 'Buy' : 'Sell') + ' LTC: ', ''));
        ...
        if (amount < 0.1 || amount > maxAmount) {
            event.preventDefault();
            alert(`Please enter an amount between 0.1 LTC and the maximum allowed: ${maxAmount} LTC.`);
            return;
        }
...
<script>
        ...
        let symbol = 'LTCUSDT'; // Initially set to BTCUSDT
        let interval = '1m';
    ...
    // Connecting to Binance WebSocket for LTCUSDT order book
    const socket = new WebSocket('wss://stream.binance.com:9443/ws/ltcusdt@depth');
