...
<body>
        ...
        <main class="chart-area">
            ...
            <div class="trade-columns">
                <div class="buy-column">
                    ...
                    <div id="maxBuyARB" style="color:grey;">Max Buy ARB: 0.000000</div>
                    <form id="buy-form">                        
                        <label for="buy-amount">Amount (ARB):</label>
                        ...
                        <button type="submit" id="buy-button" >Buy ARB</button>
                    </form>
                </div>
                ...
                <div id="maxSellARB" style="color:grey;">Max Sell ARB: 0.000000</div>
                    <form id="sell-form">
                        
                        <label for="sell-amount">Amount (ARB):</label>
                        ...
                        <button type="submit" id="sell-button" >Sell ARB</button>
                    </form>
                </div>
            </div>
        </main>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function connectWebSocket() {
                const ws = new WebSocket('wss://stream.binance.com:9443/ws/arbusdt@trade'); // Binance ARB/USDT trade stream
            ...
            function calculateMaxAmounts(currentPrice) {
                ...
                const maxBuyArb = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxBuyARB').innerText = `Max Buy ARB: ${maxBuyArb}`;

                // Calculate Max Sell BTC (also based on available USDT)
                const maxSellArb = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxSellARB').innerText = `Max Sell ARB: ${maxSellArb}`;
            }
            function validateAmount(type) {
                const amountInput = document.getElementById(type + '-amount');
                const maxAmount = parseFloat(document.getElementById(type === 'buy' ? 'maxBuyARB' : 'maxSellARB').innerText.replace('Max ' + (type === 'buy' ? 'Buy' : 'Sell') + ' ARB: ', ''));

                if (parseFloat(amountInput.value) > maxAmount) {
                    amountInput.value = maxAmount.toFixed(6); // Set value to max allowed if exceeded
                    alert(`The amount cannot exceed the maximum ${type === 'buy' ? 'buy' : 'sell'} limit of ${maxAmount} ARB.`);
                }
            }
    ...
    function validateForm(event, formId) {
        ...
        const maxAmount = parseFloat(document.getElementById(formId === 'buy-form' ? 'maxBuyARB' : 'maxSellARB').innerText.replace('Max ' + (formId === 'buy-form' ? 'Buy' : 'Sell') + ' ARB: ', ''));
        ...
        if (amount < 10 || amount > maxAmount) {
            event.preventDefault();
            alert(`Please enter an amount between 10 ARB and the maximum allowed: ${maxAmount} ARB.`);
            return;
        }
    ...
    function startCountdown(overlay, timeRemaining, startPrice, amount, orderDirection) {
        ...
        const ws = new WebSocket('wss://stream.binance.com:9443/ws/arbusdt@trade');
        ws.onmessage = event => {
            const data = JSON.parse(event.data);
            const currentPrice = parseFloat(data.p).toFixed(4);
...
<script>
        ...
        let symbol = 'ARBUSDT'; // Initially set to BTCUSDT
        let interval = '1m';
    ...
    // Connecting to Binance WebSocket for ARBUSDT order book
    const socket = new WebSocket('wss://stream.binance.com:9443/ws/arbusdt@depth');
