...
<body>
        ...
        <main class="chart-area">
            ...
            <div class="trade-columns">
                <div class="buy-column">
                    ...
                    <div id="maxBuyETC" style="color:grey;">Max Buy ETC: 0.000000</div>
                    <form id="buy-form">                        
                        <label for="buy-amount">Amount (ETC):</label>
                        ...
                        <button type="submit" id="buy-button" >Buy ETC</button>
                    </form>
                </div>
                ...
                <div id="maxSellETC" style="color:grey;">Max Sell ETC: 0.000000</div>
                    <form id="sell-form">
                        
                        <label for="sell-amount">Amount (ETC):</label>
                        ...
                        <button type="submit" id="sell-button" >Sell ETC</button>
                    </form>
                </div>
            </div>
        </main>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function connectWebSocket() {
                const ws = new WebSocket('wss://stream.binance.com:9443/ws/etcusdt@trade'); // Binance ETC/USDT trade stream
            ...
            function calculateMaxAmounts(currentPrice) {
                ...
                const maxBuyEtc = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxBuyETC').innerText = `Max Buy ETC: ${maxBuyEtc}`;

                // Calculate Max Sell BTC (also based on available USDT)
                const maxSellEtc = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxSellETC').innerText = `Max Sell ETC: ${maxSellEtc}`;
            }
            function validateAmount(type) {
                const amountInput = document.getElementById(type + '-amount');
                const maxAmount = parseFloat(document.getElementById(type === 'buy' ? 'maxBuyETC' : 'maxSellETC').innerText.replace('Max ' + (type === 'buy' ? 'Buy' : 'Sell') + ' ETC: ', ''));

                if (parseFloat(amountInput.value) > maxAmount) {
                    amountInput.value = maxAmount.toFixed(6); // Set value to max allowed if exceeded
                    alert(`The amount cannot exceed the maximum ${type === 'buy' ? 'buy' : 'sell'} limit of ${maxAmount} ETC.`);
                }
            }
    ...
    function validateForm(event, formId) {
        ...
        const maxAmount = parseFloat(document.getElementById(formId === 'buy-form' ? 'maxBuyETC' : 'maxSellETC').innerText.replace('Max ' + (formId === 'buy-form' ? 'Buy' : 'Sell') + ' ETC: ', ''));
        ...
        if (amount < 0.5 || amount > maxAmount) {
            event.preventDefault();
            alert(`Please enter an amount between 0.5 ETC and the maximum allowed: ${maxAmount} ETC.`);
            return;
        }
    ...
    function startCountdown(overlay, timeRemaining, startPrice, amount, orderDirection) {
        ...
        const ws = new WebSocket('wss://stream.binance.com:9443/ws/etcusdt@trade');
        ws.onmessage = event => {
            const data = JSON.parse(event.data);
            const currentPrice = parseFloat(data.p).toFixed(4);
...
<script>
        ...
        let symbol = 'ETCUSDT'; // Initially set to BTCUSDT
        let interval = '1m';
    ...
    // Connecting to Binance WebSocket for ETCUSDT order book
    const socket = new WebSocket('wss://stream.binance.com:9443/ws/etcusdt@depth');
