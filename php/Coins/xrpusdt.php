...
<body>
        ...
        <main class="chart-area">
            ...
            <div class="trade-columns">
                <div class="buy-column">
                    ...
                    <div id="maxBuyXRP" style="color:grey;">Max Buy XRP: 0.000000</div>
                    <form id="buy-form">                        
                        <label for="buy-amount">Amount (XRP):</label>
                        ...
                        <button type="submit" id="buy-button" >Buy XRP</button>
                    </form>
                </div>
                ...
                <div id="maxSellXRP" style="color:grey;">Max Sell XRP: 0.000000</div>
                    <form id="sell-form">
                        
                        <label for="sell-amount">Amount (XRP):</label>
                        ...
                        <button type="submit" id="sell-button" >Sell XRP</button>
                    </form>
                </div>
            </div>
        </main>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function connectWebSocket() {
                const ws = new WebSocket('wss://stream.binance.com:9443/ws/xrpusdt@trade'); // Binance XRP/USDT trade stream
            ...
            function calculateMaxAmounts(currentPrice) {
                ...
                const maxBuyXrp = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxBuyXRP').innerText = `Max Buy XRP: ${maxBuyXrp}`;

                // Calculate Max Sell BTC (also based on available USDT)
                const maxSellXrp = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxSellXRP').innerText = `Max Sell XRP: ${maxSellXrp}`;
            }
            function validateAmount(type) {
                const amountInput = document.getElementById(type + '-amount');
                const maxAmount = parseFloat(document.getElementById(type === 'buy' ? 'maxBuyXRP' : 'maxSellXRP').innerText.replace('Max ' + (type === 'buy' ? 'Buy' : 'Sell') + ' XRP: ', ''));

                if (parseFloat(amountInput.value) > maxAmount) {
                    amountInput.value = maxAmount.toFixed(6); // Set value to max allowed if exceeded
                    alert(`The amount cannot exceed the maximum ${type === 'buy' ? 'buy' : 'sell'} limit of ${maxAmount} XRP.`);
                }
            }
    ...
    function validateForm(event, formId) {
        ...
        const maxAmount = parseFloat(document.getElementById(formId === 'buy-form' ? 'maxBuyXRP' : 'maxSellXRP').innerText.replace('Max ' + (formId === 'buy-form' ? 'Buy' : 'Sell') + ' XRP: ', ''));
        ...
        if (amount < 10 || amount > maxAmount) {
            event.preventDefault();
            alert(`Please enter an amount between 10 XRP and the maximum allowed: ${maxAmount} XRP.`);
            return;
        }
...
<script>
        ...
        let symbol = 'XRPUSDT'; // Initially set to BTCUSDT
        let interval = '1m';
    ...
    // Connecting to Binance WebSocket for XRPUSDT order book
    const socket = new WebSocket('wss://stream.binance.com:9443/ws/xrpusdt@depth');
