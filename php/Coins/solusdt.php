...
<body>
        ...
        <main class="chart-area">
            ...
            <div class="trade-columns">
                <div class="buy-column">
                    ...
                    <div id="maxBuySOL" style="color:grey;">Max Buy SOL: 0.000000</div>
                    <form id="buy-form">                        
                        <label for="buy-amount">Amount (SOL):</label>
                        ...
                        <button type="submit" id="buy-button" >Buy SOL</button>
                    </form>
                </div>
                ...
                <div id="maxSellSOL" style="color:grey;">Max Sell SOL: 0.000000</div>
                    <form id="sell-form">
                        
                        <label for="sell-amount">Amount (SOL):</label>
                        ...
                        <button type="submit" id="sell-button" >Sell SOL</button>
                    </form>
                </div>
            </div>
        </main>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function connectWebSocket() {
                const ws = new WebSocket('wss://stream.binance.com:9443/ws/solusdt@trade'); // Binance SOL/USDT trade stream
            ...
            function calculateMaxAmounts(currentPrice) {
                ...
                const maxBuySol = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxBuySOL').innerText = `Max Buy SOL: ${maxBuySol}`;

                // Calculate Max Sell BTC (also based on available USDT)
                const maxSellSol = (availableUsdt / currentPrice).toFixed(6);
                document.getElementById('maxSellSOL').innerText = `Max Sell SOL: ${maxSellSol}`;
            }
            function validateAmount(type) {
                const amountInput = document.getElementById(type + '-amount');
                const maxAmount = parseFloat(document.getElementById(type === 'buy' ? 'maxBuySOL' : 'maxSellSOL').innerText.replace('Max ' + (type === 'buy' ? 'Buy' : 'Sell') + ' SOL: ', ''));

                if (parseFloat(amountInput.value) > maxAmount) {
                    amountInput.value = maxAmount.toFixed(6); // Set value to max allowed if exceeded
                    alert(`The amount cannot exceed the maximum ${type === 'buy' ? 'buy' : 'sell'} limit of ${maxAmount} SOL.`);
                }
            }
    ...
    function validateForm(event, formId) {
        ...
        const maxAmount = parseFloat(document.getElementById(formId === 'buy-form' ? 'maxBuySOL' : 'maxSellSOL').innerText.replace('Max ' + (formId === 'buy-form' ? 'Buy' : 'Sell') + ' SOL: ', ''));
        ...
        if (amount < 0.05 || amount > maxAmount) {
            event.preventDefault();
            alert(`Please enter an amount between 0.05 SOL and the maximum allowed: ${maxAmount} SOL.`);
            return;
        }
...
<script>
        ...
        let symbol = 'SOLUSDT'; // Initially set to BTCUSDT
        let interval = '1m';
    ...
    // Connecting to Binance WebSocket for SOLUSDT order book
    const socket = new WebSocket('wss://stream.binance.com:9443/ws/solusdt@depth');
