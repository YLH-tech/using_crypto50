...
if (isset($_POST["verify_email"])) {
    ...
    $sql = "UPDATE admin_users SET email_verified_at = NOW() WHERE email = '" . $email . "' AND verification_code = '" . $verification_code . "'";
    $result = mysqli_query($conn, $sql);

    if (mysqli_affected_rows($conn) == 0) {
        // Set an error message in the session
        $_SESSION['error'] = "Verification code failed. Check your email.";
        header("Location: admin_email_verfication.php?email=" . urlencode($email)); // Redirect to the same page with the email
        exit();
    }
    ...
    // Fetch the admin's last password change time to store in the session
    $sql_fetch_admin = "SELECT last_password_change FROM admin_users WHERE email = ?";
    $stmt_fetch = mysqli_prepare($conn, $sql_fetch_admin);
    mysqli_stmt_bind_param($stmt_fetch, "s", $email);
    ...
    if ($row = mysqli_fetch_assoc($result)) {
        ...
        $_SESSION['last_password_change'] = $row['last_password_change'];
    }

    // Redirect to admin dashboard or desired page
    header("Location: ../../admin/dashboard.php"); // Replace with your admin dashboard page
    exit();
}
...
if (isset($_POST["resend_code"])) {
    ...
    $sql = "UPDATE admin_users SET verification_code = '$new_verification_code' WHERE email = '$email'";
    $result = mysqli_query($conn, $sql);
    ...
    if ($result) {
        ...
        if (send_mail($email, $subject, $message)) {
            ...
            header("Location: admin_email_verfication.php?email=" . urlencode($email)); // Redirect back to the form
            exit();
        } else {
